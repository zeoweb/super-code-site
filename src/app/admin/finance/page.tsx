...
export default async function AdminFinancePage() {
  ...
  const [revenueAll, revenueMonth, expenses] = await Promise.all([
    prisma.topUp.aggregate({ where: { status: "approved" }, _sum: { amount: true } }),
  ...
  return (
    <div>
      ...
      {expenses.length === 0 ? (
        <p className="mt-2 text-sm text-slate-600">Расходов пока нет.</p>
      ) : (
          ...
          {expenses.map((e) => (
            <div key={e.id} className="card flex items-center justify-between gap-3">
              ...
              <div className="flex shrink-0 items-center gap-3">
                <span className="font-bold text-red-600">-{Number(e.amount).toLocaleString("ru-RU")} TJS</span>
                <ConfirmDeleteButton
