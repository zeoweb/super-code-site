...
export default async function TopUpMethodPage({
  ...
  return (
    <main className="mx-auto max-w-xl p-6">
      <Link href="/topup" className="text-sm text-slate-500 hover:text-slate-900">← Назад к сумме</Link>

      <CheckoutStepIndicator step={2} />

      <h1 className="mt-4 text-2xl font-bold">Способ оплаты</h1>
      <p className="mt-1 text-slate-500">
        Сумма: <strong>{amount} сомони</strong>. Шаг 2 из 3 — дальше реквизиты и чек.
      </p>
        ...
        {methods.length === 0 ? (
          <>
            <p className="text-sm text-slate-600">
              Реквизиты пока не заданы. Обратитесь в поддержку.
            </p>
            <Link href={`/topup/pay?amount=${amount}`} className="btn-ghost inline-flex">
            ...
            >
              <span className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-white">
                {m.logoUrl ? (
                  ...
                  <span className="text-xl font-bold text-slate-900">{m.bankName.charAt(0)}</span>
                )}
              </span>
              <div className="min-w-0 flex-1">
                <div className="font-bold">{m.bankName}</div>
                <div className="truncate text-sm text-slate-500">Перевод по номеру · {m.recipientName}</div>
              </div>
              <ChevronIcon className="h-5 w-5 shrink-0 text-slate-600" />
            </Link>
          ))
        )}
      </div>
