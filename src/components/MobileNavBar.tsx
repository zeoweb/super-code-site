  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex justify-center px-2 pb-4 sm:hidden">
      <div className="flex items-end gap-2">
        ...
        {/* "Играть" — отдельная акцентная кнопка (не обычный плоский пункт в
            ряду), всегда на русском (не через nav/dict), см. src/app/play.
            Иконка и подпись — внутри одной капсулы, тот же паттерн "иконка
            сверху, подпись под ней", что и у остальных 4 пунктов, просто
            крупнее. Родительский ряд выровнен по items-end (по нижнему
            краю) — при таком выравнивании margin-top на дочерний элемент
            НЕ влияет на его позицию (якорь всегда нижний край), поэтому
            центр поднимаем через margin-bottom, а не margin-top. У активного
            пункта (сейчас "Меню") нет отдельного смещения — это просто
            цветной фон на той же сетке, что и у остальных 3 (проверено
            через Playwright boundingBox: все 4 плоских пункта на одной
            высоте). Значение здесь подобрано так, чтобы именно САМА ИКОНКА
            (не капсула целиком) была на той же высоте, что иконки остальных
            4 пунктов. */}
        <Link
          href="/play"
          className={`mb-[6px] flex flex-col items-center gap-0.5 rounded-full px-3 py-2.5 text-[10px] font-bold text-white shadow-md transition-transform duration-300 hover:scale-105 ${
